...
const AmountCalcScreen = ({
  ...
  const [amount, setAmount] = useState(
    inventuries[inventuryId].items[itemId].amount,
  );
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#36393f' }}>
      <View style={{ position: 'absolute', top: 20, width: '100%' }}>
        <Text>{`${inventuryId}: ${inventuries[inventuryId].name}`}</Text>
        <Text>{`${itemId}: ${inventuries[inventuryId].items[itemId].name}`}</Text>
      </View>
      <View
        style={{
          position: 'absolute',
            ...
            onPress={() => {
              try {
                if (amount != '') {
                  ...
                  if (confirmed) {
                    dispatch(
                      itemSetAmount({ inventuryId, itemId, newAmount: amount }),
                    );
                    navigation.goBack();
                  } else {
                    setConfirmed(true);
